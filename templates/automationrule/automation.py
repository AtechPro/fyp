...
@autobp.route('/automation/rules/<int:rule_id>', methods=['GET'])
def get_automation_rule(rule_id):
    rule = AutomationRule.query.get_or_404(rule_id)
    return jsonify(rule.as_dict()), 200


@autobp.route('/automation/rules/<int:rule_id>', methods=['PUT'])
def edit_automation_rule(rule_id):
    rule = AutomationRule.query.get_or_404(rule_id)
    data = request.get_json()

    # Update the rule attributes
    rule.user_id = data.get('user_id', rule.user_id)
    rule.sensor_id = data.get('sensor_id', rule.sensor_id)
    rule.sensor_type_id = data.get('sensor_type_id', rule.sensor_type_id)
    rule.condition = data.get('condition', rule.condition)
    rule.threshold = data.get('threshold', rule.threshold)
    rule.relay_device_id = data.get('relay_device_id', rule.relay_device_id)
    rule.action = data.get('action', rule.action)
    rule.enabled = data.get('enabled', rule.enabled)
    rule.auto_title = data.get('auto_title', rule.auto_title)
    rule.auto_description = data.get('auto_description', rule.auto_description)

    db.session.commit()
    return jsonify(rule.as_dict()), 200

# Delete Route
@autobp.route('/automation/rules/<int:rule_id>', methods=['DELETE'])
